...
# Editing /etc/rhsm/rhsm.conf to use zscaler proxy to connect to RedHAT CDN
echo "**** Editing /etc/rhsm/rhsm.conf to use proxy ****"
echo
sleep 3
cat <<'EOF' | sudo tee /rhsm/rhsm.conf
...
options=("1: TPC Subscription" "2: Developer Subscription" "3: Add custom subscription pool ID" "Quit")
select opt in "${options[@]}"
do
    case $opt in
        "1: TPC Subscription")
            echo "**** Subscribing to the account: InfraOPS_5671963 ****"
            echo
            subscription-manager register --username "InfraOPS_5671963"
            subscription-manager attach --pool=2c94dcd786db653a0186e0e4f1743789
            echo -e "Done\n"
            break;;

        "2: Developer Subscription")
            echo "**** Subscribing to the account: InfraOPS_5671963 ****"
            echo
            subscription-manager register --username "InfraOPS_5671963"
            subscription-manager attach --pool=2c94760e887329bc0188b51bc1fa366e
            echo -e "Done\n"
            break;;

        "3: Add custom subscription pool ID")
            echo "**** Subscribing to the account: InfraOPS_5671963 ****"
            echo
            subscription-manager register --username "InfraOPS_5671963" 
            echo "**** Please add the subscription pool ID: ****"
            read = $poolid
